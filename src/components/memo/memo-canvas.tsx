...
import { ArrowLeft, X, FileText, Download, Share2, ChevronRight, History, MessageSquare } from 'lucide-react';
...
export function MemoCanvas({ deal, onClose }: MemoCanvasProps) {
    const { memo, isGenerating, comments } = useMemo();
    ...
    return (
        <div className="flex flex-col h-full bg-white animate-in fade-in slide-in-from-left-4 duration-300">
                    ...
                    {memo && (
                        <>
                            <Button
                                ...
                                className="h-8 text-slate-600 hover:text-slate-900 relative"
                                title="Comments"
                            >
                                ...
                                {comments.length > 0 && (
                                    <Badge
                                        variant="secondary"
                                        className="ml-1.5 h-4 px-1 text-[10px] min-w-[16px] justify-center"
                                    >
                                        {comments.length}
                                    </Badge>
                                )}
            ...
            <div className="flex-1 overflow-hidden bg-white flex">
                ...
                {memo && isCommentsPanelOpen && (
                    <div className="w-80 flex-shrink-0 animate-in slide-in-from-right-4 duration-300">
                        <CommentsPanel onClose={() => setIsCommentsPanelOpen(false)} />
                    </div>
                )}
            </div>
